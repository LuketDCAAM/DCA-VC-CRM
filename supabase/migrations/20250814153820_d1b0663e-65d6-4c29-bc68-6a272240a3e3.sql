-- Drop and recreate the find_potential_duplicates function with correct return type
-- The issue is that confidence_score needs to be real, not numeric

DROP FUNCTION IF EXISTS public.find_potential_duplicates(text, text, text, text, uuid);
...
RETURNS TABLE(deal_id uuid, company_name text, website text, linkedin_url text, contact_email text, contact_name text, pipeline_stage text, created_at timestamp with time zone, confidence_level text, confidence_score real, match_reasons text[])
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      WHEN LOWER(TRIM(d.company_name)) = normalized_company_name THEN 1.0::real
      WHEN p_website IS NOT NULL AND d.website IS NOT NULL AND 
           LOWER(REGEXP_REPLACE(d.website, '^https?://(www\.)?', '')) = website_domain THEN 0.95::real
      WHEN p_linkedin_url IS NOT NULL AND d.linkedin_url IS NOT NULL AND 
           LOWER(d.linkedin_url) = LOWER(p_linkedin_url) THEN 0.9::real
      WHEN p_contact_email IS NOT NULL AND d.contact_email IS NOT NULL AND 
           LOWER(d.contact_email) = LOWER(p_contact_email) THEN 0.85::real
