...
-- Drop the existing table and enum if they exist
DROP TABLE IF EXISTS public.lp_engagements CASCADE;
DROP TYPE IF EXISTS engagement_stage CASCADE;

-- Create engagement stages enum first
...
-- Create LP engagements table with proper enum type
CREATE TABLE public.lp_engagements (
  ...
  engagement_stage engagement_stage DEFAULT 'Prospect'::engagement_stage,
  location TEXT,
